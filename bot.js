...
const antiVenom = new Discord.Client()
antiVenom.on('ready', () => {
    console.log(`Logged in as ${antiVenom.user.tag}!`)
})
antiVenom.login(DISCORD_TOK)
...
antiVenom.on('message', async (msg) => {
    try {
        ...
        console.log('antiVenom message: ' + e)
...
async function connect(msg, mapKey) {
    try {
        let voice_Channel = await antiVenom.channels.fetch(msg.member.voice.channelID);
        if (!voice_Channel) return msg.reply("Error: The voice channel does not exist!");
        let text_Channel = await antiVenom.channels.fetch(msg.channel.id);
